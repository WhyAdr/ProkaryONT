...
#                           (NOTE: currently a no-op — RagTag was removed
#                           from 03_autocycler_assemble.sh pending a
#                           dedicated post-assembly scaffolding script)
...
Subcommands:
  assemble      Run the full assembly pipeline (Stages 1-4)
  taxonomy      Run taxonomy assignment (05_taxonomy.sh)
  annotate      Run Bakta annotation and QA (06_annotate_assess.sh)
...
case "${SUBCOMMAND}" in
    ...
    taxonomy)
        ...
        bash "${SCRIPT_DIR}/05_taxonomy.sh" --config "${CONFIG_FILE}"
        ;;
    annotate)
        [[ -z "${CONFIG_FILE}" ]] && { echo "ERROR: --config is required for 'annotate'" >&2; exit 1; }
        bash "${SCRIPT_DIR}/06_annotate_assess.sh" --config "${CONFIG_FILE}"
        ;;
    *)
